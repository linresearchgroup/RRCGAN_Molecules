...
encoder = load_model('./../data/nns/encoder_newencinp.h5')
decoder = load_model('./../data/nns/decoder_newencinp.h5')
...
#generator = load_model    ('./../data/nns/keep_yxie/generator_new.h5')
#discriminator= load_model ('./../data/nns/keep_yxie/discriminator_new.h5')
...
for e in range(epochs):
    ...
    for iter_, smiles in enumerate(gen_smiles):
        ...
        if m0 is not None:
            ...
            idx0_.append(iter_)
        if m is not None:
            if len(construct_atomic_number_array(m)) <= 10:
                valid += 1
                idx_.append(iter_)
                try:
...
generator.save('./../data/nns/generator_ups10HA.h5')
discriminator.save('./../data/nns/discriminator_ups10HA.h5')
...
rsquared = r2_score (output['pred_cv'], output['des_cv'])
print ("r squared r**2, pred gound thruth", rsquared)
...
output2.to_csv('./../experiments/regular/Initial_training.csv', index = False)
...
print ('% < 20 RE', sum (output['Err_pred_des'] < 0.2) / output['Err_pred_des'].shape[0])
explained_variance_R2_pred_des = explained_variance_score(output['des_cv'], output['pred_cv'])
print ("explained_varice_R2_pred_des", explained_variance_R2_pred_des)
rsquared = r2_score (output['des_cv'], output['pred_cv'])
print ("r squared r**2", rsquared)

...
X_train, X_test, y_cv_train, y_cv_test, y_E_homo_train, y_E_homo_test,y_E_lumo_train, y_E_lumo_test,y_gap_train, y_gap_test,y_r2_train, y_r2_test, y_zpve_train, y_zpve_test, y_u0_train, y_u0_test,y_u298_train, y_u298_test,y_h298_train, y_h298_test,y_g298_train, y_g298_test = train_test_split( data.iloc[:,0:17].values, data.iloc[:,18].values, data.iloc[:,19].values, data.iloc[:,20].values, data.iloc[:,21].values, data.iloc[:,22].values, data.iloc[:,23].values, data.iloc[:,24].values, data.iloc[:,25].values, data.iloc[:,26].values, data.iloc[:,27].values)
...
"""
properties_train = [ y_cv_train, y_E_homo_train, y_E_lumo_train, y_gap_train, y_r2_train, y_zpve_train, y_u0_train, y_u298_train, y_h298_train, y_g298_train]
properties_test = [y_cv_test, y_E_homo_test, y_E_lumo_test, y_gap_test, y_r2_test, y_zpve_test, y_u0_test, y_u298_test, y_h298_test, y_g298_test]  
#Default parameters
count = 1
for i,j in zip(properties_train, properties_test): 
 start = time.time()
 y_train = i
 y_test = j
 ...
 #print the results
 print ("test r-square from the model: ",regr.score ( X_test, y_test), "for: " ,count)
 print ("test r-square from the model: ",r2_score ( y_test, y_pred), "for: " ,count )
 print ("train r-square from the model: ",regr.score (X_train, y_train),"for: ", count)
 print ("train r-square from the model: ",r2_score (y_train, y_pred_train),"for: ", count)

 print ("Mean of {}: {} ".format(count,np.mean(y_test)))
 print ("Mean Squared Error: ",mean_squared_error(y_test, y_pred))
 print ("Mean Squared Error train: ",mean_squared_error(y_train, y_pred_train))
 print ("MSE/Mean_test: ", mean_squared_error(y_test, y_pred)/np.mean(y_test))
 print ("Mean absolute error: ",mean_absolute_error(y_test, y_pred))
 print ("Mean_absolute_error/Mean_test: ",mean_absolute_error(y_test, y_pred)/np.mean(y_test))

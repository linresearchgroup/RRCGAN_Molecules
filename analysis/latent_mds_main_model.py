...
embedding = MDS(n_components=2, n_jobs=-1, max_iter=1000)
X_concat_train_MDS = embedding.fit_transform(X_Concat_train)
X_concat_test_MDS = embedding.fit_transform(X_Concat_test)
...
mpl.rcParams['axes.linewidth'] = 3.5
for i, c, label in zip(target_ids, colors, group_names):
            plt.scatter(X_concat_test_MDS[y_class_val == i, 0],
                        X_concat_test_MDS[y_class_val == i, 1],
                        alpha=0.5, c=c, label=label)
plt.legend(fontsize=12)
ax.tick_params(width=2, length=4)
plt.xlabel('MDS1', fontsize=25, fontweight='bold')
plt.ylabel('MDS2', fontsize=25, fontweight='bold')
plt.savefig("test_conc_dist_MDS.png", bbox_inches='tight', dpi=300)
...
mpl.rcParams['axes.linewidth'] = 3.5
for i, c, label in zip(target_ids, colors, group_names):
            plt.scatter(X_concat_train_MDS[y_class == i, 0],
                        X_concat_train_MDS[y_class == i, 1],
                        alpha=0.5, c=c, label=label)
plt.legend(fontsize=12)
ax.tick_params(width=2, length=4)
plt.xlabel('MDS1', fontsize=25, fontweight='bold')
plt.ylabel('MDS2', fontsize=25, fontweight='bold')
plt.savefig("train_conc_dist_MDS.png", bbox_inches='tight', dpi=300)

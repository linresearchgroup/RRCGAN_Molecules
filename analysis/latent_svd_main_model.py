...
X_concat_train_SVD = csr_matrix(X_Concat_train)
X_concat_test_SVD = csr_matrix(X_Concat_test)
svd = TruncatedSVD(n_components=5, n_iter=7, random_state=42)
X_concat_train_SVD = svd.fit_transform(X_concat_train_SVD)
X_concat_test_SVD = svd.fit_transform(X_concat_test_SVD)
...
mpl.rcParams['axes.linewidth'] = 3.5
for i, c, label in zip(target_ids, colors, group_names):
            plt.scatter(X_concat_test_SVD[y_class_val == i, 0],
                        X_concat_test_SVD[y_class_val == i, 1],
                        alpha=0.5, c=c, label=label)
plt.legend(fontsize=12)
ax.tick_params(width=2, length=4)
plt.xlabel('SVD1', fontsize=25, fontweight='bold')
plt.ylabel('SVD2', fontsize=25, fontweight='bold')
plt.savefig("test_conc_dist_SVD.png", bbox_inches='tight', dpi=300)
...
mpl.rcParams['axes.linewidth'] = 3.5
for i, c, label in zip(target_ids, colors, group_names):
            plt.scatter(X_concat_train_SVD[y_class == i, 0],
                        X_concat_train_SVD[y_class == i, 1],
                        alpha=0.5, c=c, label=label)
plt.legend(fontsize=12)
ax.tick_params(width=2, length=4)
plt.xlabel('SVD1', fontsize=25, fontweight='bold')
plt.ylabel('SVD2', fontsize=25, fontweight='bold')
plt.savefig("train_conc_dist_SVD.png", bbox_inches='tight', dpi=300)

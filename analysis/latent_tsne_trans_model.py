...
with open('./../experiments/regular_9HA_6b6latent/latent/gen_atoms_bonds3.pickle', 'wb') as f:
    pickle.dump((gen_atoms_embedding, gen_bonds_embedding), f)

output = pd.DataFrame(output)
output.reset_index(drop = True, inplace = True)
output.to_csv ('./../experiments/regular_9HA_6b6latent/latent/Regular_noscreenrelug3.csv', index=False)
"""
...
preds_ = gen_SMILES ['pred_cv']
gen_error = gen_SMILES ['Err_pred_des']

with open('./../experiments/regular_9HA_6b6latent/latent/gen_atoms_bonds2.pickle', 'rb') as f:
    gen_atoms_embedding_, gen_bonds_embedding_ = pickle.load(f)

with open('./../experiments/regular_9HA_6b6latent/latent/gen_atoms_bonds_trans.pickle', 'rb') as f:
    gen_atoms_embedding_trans, gen_bonds_embedding_trans = pickle.load(f)

csv_name = './../experiments/regular_9HA_6b6latent/latent/trans2_noscreen.csv'
gen_SMILES_trans = pd.read_csv(csv_name)
gen_smiles_trans = gen_SMILES_trans ['SMILES']
sample_ys_trans = gen_SMILES_trans ['des_cv']
preds_trans = gen_SMILES_trans ['pred_cv']
gen_error_trans = gen_SMILES_trans ['Err_pred_des']

gen_atoms_embedding_trans = np.array(gen_atoms_embedding_trans)
gen_bonds_embedding_trans = np.array(gen_bonds_embedding_trans)

# subsampling
idx = np.random.choice(len(preds_trans), int(len(preds_trans) * 0.07), replace = False)
preds_trans, gen_smiles_trans = (preds_trans[idx], gen_smiles_trans[idx])
gen_atoms_embedding_trans = gen_atoms_embedding_trans[idx]
gen_bonds_embedding_trans = gen_bonds_embedding_trans[idx]

preds = np.concatenate([preds_, preds_trans], axis=0)
print ('preds shape', preds.shape)
print ('preds_trans', preds_trans.shape)
print ('des cv', sample_ys)


gen_atoms_embedding_ = np.array(gen_atoms_embedding_)
gen_bonds_embedding_ = np.array(gen_bonds_embedding_)

with open('./../data/trainingsets/highcv_DFT/highcv_DFT.pickle', 'rb') as f:
    X_smiles_gantrain_trans, SMILES_gantrain_trans, cv_gantrain_trans = pickle.load(f)
# subsampling
idx = np.random.choice(len(cv_gantrain_trans), int(len(cv_gantrain_trans) * 0.18), replace = False)
cv_gantrain_trans, SMILES_gantrain_trans, X_smiles_gantrain_trans = (cv_gantrain_trans[idx], 
                                                                     SMILES_gantrain_trans[idx], 
                                                                     X_smiles_gantrain_trans[idx])

encoder_trans = load_model('./../data/nns_9HA_noemb_6b6/encoder_newencinp_trans2.h5')
decoder_trans = load_model('./../data/nns_9HA_noemb_6b6/decoder_newencinp_trans2.h5')
atoms_embedding_trans, bonds_embedding_trans, _ = encoder_trans.predict([X_smiles_gantrain_trans])


# create classes for heat capacity
# using cv_gantrain, uniformly distributed.
y_train_ = cv_gantrain * (s_max_norm - s_min_norm) + s_min_norm
print ('y_train shape', y_train_.shape)
print ('cv_gantrain_trans shape', cv_gantrain_trans.shape)

y_train = np.concatenate ([y_train_, cv_gantrain_trans], axis=0)
print (y_train.shape)
...
print ("quantile of gen samples: ", Qs_gen)


#############for 10 classes, activate this block
...
y_class_val = np.where(preds <= Qs_gen[0], 0, y_class_val)
y_class_val = np.where((preds > Qs_gen[0]) & (preds <= Qs_gen[1]), 1, y_class_val)
y_class_val = np.where((preds > Qs_gen[1]) & (preds <= Qs_gen[2]), 2, y_class_val)
y_class_val = np.where((preds > Qs_gen[2]) & (preds <= Qs_gen[3]), 3, y_class_val)
y_class_val = np.where((preds > Qs_gen[3]) & (preds <= Qs_gen[4]), 4, y_class_val)
y_class_val = np.where((preds > Qs_gen[4]) & (preds <= Qs_gen[5]), 5, y_class_val)
y_class_val = np.where((preds > Qs_gen[5]) & (preds <= Qs_gen[6]), 6, y_class_val)
y_class_val = np.where((preds > Qs_gen[6]) & (preds <= Qs_gen[7]), 7, y_class_val)
y_class_val = np.where((preds > Qs_gen[7]) & (preds <= Qs_gen[8]), 8, y_class_val)
y_class_val = np.where(preds > Qs_gen[8], 9, y_class_val)
...
print ('gen class==4', sum(y_class_val == 4))
print ('gen class==5', sum(y_class_val == 5))
print ('gen class==6', sum(y_class_val == 6))
print ('gen class==7', sum(y_class_val == 7))
print ('gen class==8', sum(y_class_val == 8))
print ('gen class==9', sum(y_class_val == 9))
...
print ('train class==4', sum(y_class==4))
print ('train class==5', sum(y_class==5))
print ('train class==6', sum(y_class==6))
print ('train class==7', sum(y_class==7))
print ('train class==8', sum(y_class==8))
print ('train class==9', sum(y_class==9))
##############################

"""
######## 4 classes, activate this block
...
# ANALYSIS
train_atoms_embedding_, train_bonds_embedding_, _ = encoder.predict([X_smiles_gantrain]) 
train_atoms_embedding = np.concatenate ([train_atoms_embedding_, atoms_embedding_trans], axis=0)
train_bonds_embedding = np.concatenate ([train_bonds_embedding_, bonds_embedding_trans], axis=0)

gen_atoms_embedding = np.concatenate ([gen_atoms_embedding_, gen_atoms_embedding_trans], axis=0)
gen_bonds_embedding = np.concatenate ([gen_bonds_embedding_, gen_bonds_embedding_trans], axis=0)
...
###### for 10 clusters activate this block
...
"""
##########for 4 clusters, activate this block
...
SMILES_gantrain_concat =  np.concatenate ([SMILES_gantrain, SMILES_gantrain_trans], axis=0)
# using .csv file to save mapped vectors
tsne_components = pd.DataFrame()
tsne_components['SMILES'] = SMILES_gantrain_concat
...
tsne_components.to_csv('./TSNE_variables_train_trans0.csv')

gen_smiles_concat = np.concatenate ([gen_smiles, gen_smiles_trans], axis=0)
tsne_components = pd.DataFrame()
tsne_components['SMILES'] = gen_smiles_concat
...
tsne_components.to_csv('./TSNE_variables_test_trans0.csv')

SMILES_gantrain_concat =  np.concatenate ([SMILES_gantrain, SMILES_gantrain_trans], axis=0)
gen_smiles_concat = np.concatenate ([gen_smiles, gen_smiles_trans], axis=0)

tsne_components = pd.read_csv('./TSNE_variables_train_trans0.csv')
X_Concat_train_tsne1 = np.array (tsne_components['train_tsne1'])
X_Concat_train_tsne2 = np.array (tsne_components['train_tsne2'])

tsne_components = pd.read_csv('./TSNE_variables_test_trans0.csv')
...
print ('train SMILES from class {} and id {}'.format(class_sample, id_sample), 
		SMILES_gantrain_concat[y_class == class_sample][id_sample])
print ('train Cv from class {} and id {}'.format(class_sample, id_sample),
		y_train [y_class == class_sample][id_sample])

plt.legend(fontsize=20, loc='lower left', bbox_to_anchor=(0.8, 0.62))
ax.tick_params(width=2, length=4)

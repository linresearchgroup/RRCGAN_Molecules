...
idx = np.random.choice(len(y_train), int(len(y_train) * 0.03), replace = False)

X_smiles_train, SMILES_train, X_atoms_train, X_bonds_train, y_train = (X_smiles_train[idx],
                                                SMILES_train[idx],
                                                X_atoms_train[idx],
                                                X_bonds_train[idx],
                                                y_train[idx])
...
idx = np.random.choice(len(y_test), int(len(y_test) * 0.01), replace = False)

X_smiles_test, SMILES_test, X_atoms_test, X_bonds_test, y_test = (X_smiles_test[idx],
                                                     SMILES_test[idx],
                                                     X_atoms_test[idx],
                                                     X_bonds_test[idx],
                                                     y_test[idx])
...
print (len(y_train))
print (len(y_test))
with open('./image_train_demo.pickle', 'wb') as f:
    pickle.dump((X_smiles_train, SMILES_train, X_atoms_train, X_bonds_train, y_train), f)
    
with open('./image_test_demo.pickle', 'wb') as f:

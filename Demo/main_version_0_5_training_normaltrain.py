...
with open('./image_train_demo.pickle', 'rb') as f:
    X_smiles_train, SMILES_train, X_atoms_train, X_bonds_train, y_train = pickle.load(f)
    
with open('./image_test_demo.pickle', 'rb') as f:
...
encoder = load_model('./encoder_newenc.h5')
decoder = load_model('./decoder_newenc.h5')
...
# Train
pred = regressor.predict([atoms_embedding, bonds_embedding], verbose=0)
...
# Test
pred = regressor.predict([atoms_val, bonds_val], verbose=0)
...
regressor = load_model    ('./regressor.h5')
regressor_top = load_model('./regressor_top.h5')
#generator = load_model    ('./generator_new.h5')
#discriminator= load_model ('./discriminator_new.h5')
...
#regressor.save('regressor.h5')
#regressor_top.save('regressor_top.h5')
generator.save('./generator_new.h5')
discriminator.save('./discriminator_new.h5')
...
output2.to_csv('./demo_NODUP.csv', index = False)
